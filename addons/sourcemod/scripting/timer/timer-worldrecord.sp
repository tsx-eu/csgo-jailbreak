...
DeleteMapRecordsMenu(client)
{
	if (0 < client < MaxClients)
	{
		...
		SetMenuTitle(menu, "Vous êtes sûr ?");
		AddMenuItem(menu, "no", "POURQUOI ?");
		AddMenuItem(menu, "no", "Coucou toi :3");
		AddMenuItem(menu, "no", "J'sais pas");
		AddMenuItem(menu, "no", "Et c'est tout !");
		AddMenuItem(menu, "yes", "*** Absolument sûr !***");
...
CreateWRMenu(client, style, track)
{
	...
	if(track == TRACK_NORMAL)
	{
		...
		SetMenuTitle(menu, "Records sur %s [%d total]", g_currentMap, total);
	}
	else if (track == TRACK_BONUS)
	{
		menu = CreateMenu(MenuHandler_BonusWR);
		SetMenuTitle(menu, "Bonus-records sur %s [%d total]", g_currentMap, total);
	}
	else if (track == TRACK_BONUS2)
	{
...
CreateRecordInfoMenu(client, index, track)
{
	...
	FormatEx(text, sizeof(text), "Joueur: %s (%s)", nCache[Name], nCache[Auth]);
	AddMenuItem(menu, sStyle, text);

	FormatEx(text, sizeof(text), "Position: #%d [FC: %d]", index+1, nCache[FinishCount]);
	AddMenuItem(menu, sStyle, text);

	FormatEx(text, sizeof(text), "Temps: %s", nCache[TimeString]);
	AddMenuItem(menu, sStyle, text);

	FormatEx(text, sizeof(text), "Vitesse [Avg: %.2f | Max: %.2f | Fin: %.2f]", nCache[AvgSpeed], nCache[MaxSpeed], nCache[FinishSpeed]);
	AddMenuItem(menu, sStyle, text);

	if (g_Settings[JumpsEnable])
	{
		FormatEx(text, sizeof(text), "Sauts: %d", nCache[Jumps]);
