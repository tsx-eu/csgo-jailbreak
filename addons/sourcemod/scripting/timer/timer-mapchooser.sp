...
public OnMapStart()
{
	...
	if (strcmp(folder, "csgo") == 0 && GetConVarInt(g_Cvar_GameType) == GameType_GunGame &&
...
public OnConfigsExecuted()
{
	...
	// Disable the next level vote in CS:GO
...
InitiateVote(MapChange:when, Handle:inputlist=INVALID_HANDLE)
{
	...
	else
	{
		...
		SetMenuTitle(g_VoteMenu, "Votez pour la prochaine map");
	...
	else //We were given a list of maps to start the vote with
	{
		...
		for (new i=0; i<size; i++)
		{
			...
			else if (StrEqual(map, VOTE_DONTCHANGE))
			{
				if (g_NativeVotes)
				{
					NativeVotes_AddItem(g_VoteMenu, VOTE_DONTCHANGE, "Ne pas changer");
				}
				else
				{
					AddMenuItem(g_VoteMenu, VOTE_DONTCHANGE, "Ne pas changer");
				}
			}
			else if (StrEqual(map, VOTE_EXTEND)) {
				if (g_NativeVotes)
				{
					NativeVotes_AddItem(g_VoteMenu, VOTE_EXTEND, "Rallonger la carte");
				}
				else
				{
					AddMenuItem(g_VoteMenu, VOTE_EXTEND, "Rallonger la carte");
				}
			}
		}
	...
	switch (action)
	{
		...
		case MenuAction_DisplayItem:
		{
			...
			else if (!StrEqual(map, LINE_SPACER, false))
			{
				...
				ReplaceString(map, sizeof(map), "surf_", "", false);
				ReplaceString(map, sizeof(map), "bhop_", "", false);
				ReplaceString(map, sizeof(map), "bh_", "", false);
				ReplaceString(map, sizeof(map), "mg_", "", false);
				...
				if(!InternalIsMapOfficial(map))
				{
					if(tier > 0)
					{
						switch (mark)
						{
							case 1:
							{
								Format(buffer, sizeof(buffer), "[T:%d] %T", "Custom Marked", param1, tier, map);
							}

							case 2:
							{
								Format(buffer, sizeof(buffer), "[T:%d] %T", "Custom", param1, tier, map);
							}
						}
					}
					else
...
stock AddExtendToMenu(Handle:menu, MapChange:when)
{
	...
	if ((when == MapChange_Instant || when == MapChange_RoundEnd) && GetConVarBool(g_Cvar_DontChange))
	{
		if (g_NativeVotes)
		{
			...
			NativeVotes_AddItem(menu, VOTE_DONTCHANGE, "Ne pas changer");
		}
		else
		{
			AddMenuItem(menu, VOTE_DONTCHANGE, "Ne pas changer");
		}
	}
	else if (GetConVarBool(g_Cvar_Extend) && g_Extends < GetConVarInt(g_Cvar_Extend))
	{
		if (g_NativeVotes)
		{
			NativeVotes_AddItem(menu, VOTE_EXTEND, "Rallonger la carte");
		}
		else
		{
			AddMenuItem(menu, VOTE_EXTEND, "Rallonger la carte");
		}
	}
}
