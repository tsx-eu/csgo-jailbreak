...
public OnPluginStart()
{
	...
	RegAdminCmd("timer_setrankpoints", Command_SetRankPoints, ADMFLAG_ROOT, "Usage: timer_setrankpoints <steam> <amount> | <steam> must exist otherwise the operation fails.");
	RegAdminCmd("timer_changerankpoints", Command_ChangeRankPoints, ADMFLAG_ROOT, "Usage: timer_changerankpoints <steam> <amount> | <steam> must exist otherwise the operation fails. | Positive to add, Negative to subtract.");
	RegAdminCmd("timer_listranks", Command_ListRanks, ADMFLAG_ROOT, "Queries the database for all ranking information and displays it to server console or issuing admin.");
...
public OnClientDisconnect(client)
{
	...
	if(g_bAuthed[client])
	{
		...
		if(g_iPositionMethod == 2 || points <= 0)
		{
			...
			CPrintToChatAll("%s{white}%s {blue}s'est déconnecté.", sNameBuffer, g_sName[client]);
			#else
		...
		else
		{
			#if defined LEGACY_COLORS
			CFormat(sNameBuffer, 1024, client);
			CPrintToChatAll("%s{white}%s {blue}s'est déconnecté avec {white}%d {blue}points ({white}%s%d{blue}).", sNameBuffer, g_sName[client], points, sPre, points-points_start);
			#else
			CReplaceColorCodes(sNameBuffer, client, false, 1024);
...
stock PrintToAdmins(const String:format[], any:...)
{
	...
	for(new i = 1; i <= MaxClients; i++)
	{
		...
		CPrintToChat(i, "%s%s", PREFIX, sBuffer);
	}
