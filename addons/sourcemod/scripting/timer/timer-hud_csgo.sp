...
ShowHudMenu(client, start_item)
{
	if(g_Settings[HUDMasterOnlyEnable] && g_Settings[HUDMasterEnable])
	{
		...
		{
			hudSettings[Master][client] = 0;
			CPrintToChat(client, "%s HUD désactivé.", PREFIX);
		}
		else
		{
			hudSettings[Master][client] = 1;
			CPrintToChat(client, "%s HUD activé.", PREFIX);
		}
	}
	else if(g_Settings[HUDMasterEnable])
	{
		...
		FormatEx(buffer, sizeof(buffer), "Menu de l'HUD");
		...
		if(hudSettings[Time][client] == 0)
		{
			AddMenuItem(menu, "time", "[V] Temps");
		}
		else
		{
			AddMenuItem(menu, "time", "[X] Temps");
		}
		...
		if(g_Settings[HUDJumpsEnable])
		{
			if(hudSettings[Jumps][client] == 0)
			{
				AddMenuItem(menu, "jumps", "[V] Sauts");
			}
			else
			{
				AddMenuItem(menu, "jumps", "[X] Sauts");
			}
		}
		...
		if(g_Settings[HUDSpeedEnable])
		{
			if(hudSettings[Speed][client] == 0)
			{
				AddMenuItem(menu, "speed", "[V] Vitesse");
			}
			else
			{
				AddMenuItem(menu, "speed", "[X] Vitesse");
			}
		}
		...
		if(g_Settings[HUDSpeedMaxEnable])
		{
			if(hudSettings[SpeedMax][client] == 0)
			{
				AddMenuItem(menu, "speedmax", "[V] Vitesse max");
			}
			else
			{
				AddMenuItem(menu, "speedmax", "[X] Vitesse max");
			}
		}
...
UpdateHUD_CSGO(client)
{
	...
	*/
	...
	if (hudSettings[Level][client] && g_Settings[MultimodeEnable])
	{
		if(stagecount <= 1)
		{
			Format(centerText, sizeof(centerText), "%sNiveau: Linear", centerText, currentLevel, stagecount);
		}
		else
		{
			Format(centerText, sizeof(centerText), "%sNiveaux: %d/%d", centerText, currentLevel, stagecount);
		}
	...
	if(hudSettings[Time][client])
	{
		...
		{
			Format(centerText, sizeof(centerText), "%sTemps: <font color='FF8A00'>Paused</font>", centerText, timeString);
		}
		else if (enabled)
		{
			if(RecordTime == 0.0 || RecordTime > time)
			{
				Format(centerText, sizeof(centerText), "%sTemps: <font color='#00ff00'>%s</font>", centerText, timeString);
			}
			else
			{
				Format(centerText, sizeof(centerText), "%sTemps: <font color='#ff0000'>%s</font>", centerText, timeString);
			}
		}
		else Format(centerText, sizeof(centerText), "%sTemps: <font color='#ff0000'>Stopped</font>", centerText);
	...
	if(ranked && g_timerWorldRecord)
	{
		...
		{
			if(rank < 1)
				Format(centerText, sizeof(centerText), "%sPosition: -/%s", centerText, sRankTotal);
			else
				Format(centerText, sizeof(centerText), "%sPosition: %d/%s", centerText, rank, sRankTotal);
		...
		if(hudSettings[PB][client])
		{
			...
			Format(centerText, sizeof(centerText), "%sRecord perso': %s", centerText, bestbuffer);
		}

		if(hudSettings[PB][client] || hudSettings[Rank][client])
			Format(centerText, sizeof(centerText), "%s\n", centerText);
	}
	else Format(centerText, sizeof(centerText), "%sFUN - Non ranké\n", centerText);
	...
	else if (hudSettings[Level][client] && !g_Settings[MultimodeEnable])
	{
		if(stagecount <= 1)
		{
			Format(centerText, sizeof(centerText), "%sNiveau: Linéaire", centerText, currentLevel, stagecount);
		}
		else
		{
			Format(centerText, sizeof(centerText), "%sNiveau: %d/%d", centerText, currentLevel, stagecount);
		}
	...
	if(hudSettings[Speed][client])
		Format(centerText, sizeof(centerText), "%sVitesse: %5.2f", centerText, currentspeed);
...
Print_Specinfo(client, owner)
{
	...
	CPrintToChat(owner, "%s {white}%N {blue}a {white}%d {blue}spectateurs : {white}%s{blue}.", PREFIX, client, count, buffer);
}
