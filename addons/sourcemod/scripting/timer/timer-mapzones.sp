...
bool g_bRun[MAXPLAYERS + 1];
int g_Start[MAXPLAYERS + 1];
bool g_bMapBlacklist = false;
...
public void LoadBlacklist() {
	g_bMapBlacklist = false;

	File file = OpenFile("addons/sourcemod/configs/timer/runretry_blacklist.ini", "r"); 
	if((file == null)) { 
	...
	char szLine[60]; 
	char szMap[32];
	GetCurrentMap(szMap, 32);
	
	while(file.ReadLine(szLine, sizeof(szLine)))
	{ 
		TrimString(szLine);
		PrintToChatAll(szLine);
		
		if(StrEqual(szLine, szMap)) {
			g_bMapBlacklist = true;
		}
	} 
	delete file; 
}
...
public Action:StartTouchTrigger(caller, activator)
{
	...
	else if (g_mapZones[zone][Type] == ZtStart)
	{
		if(!g_bMapBlacklist) {
			if(g_bRun[client]) {
				if(GetTime() - g_Start[client] <= TIME_RESTART) {
